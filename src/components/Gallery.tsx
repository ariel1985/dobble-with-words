...
function Gallery({ isOpen, toggle, loadUrls }) {
    ...
    return (
        <Container >
            <GalleryHeader icon={{ name: 'images' }} text="Free pictures to add to your cards - מאגר תמונות לעיצוב הקלפים">
                <Dropdown text='Images gallery \ בחירה מהמאגר'
                    options={options}
            ...
            <Container style={{ padding: '2em' }}>
                <p>
                    {!!images.length && <Card.Group itemsPerRow={6}>
                        {images.map((url) =>
                            <Card
                                onClick={() => loadUrls([url])}
                                image={url} raised />
                        )}
                    </Card.Group>}
                </p>
